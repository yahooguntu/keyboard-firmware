...
void MCP::pinMode(uint8_t pin, uint8_t mode) {  // Accept the pin # and I/O mode
  if (pin > 15) return;                         // If the pin value is not valid (1-16) return, do nothing and return
  if (mode == HIGH) {                           // Determine the mode before changing the bit state in the mode cache
...
uint8_t MCP::digitalRead(uint8_t pin) {                    // Return a single bit value, supply the necessary bit (1-16)
    if (pin > 15) return 0x0;                    // If the pin value is not valid (1-16) return, do nothing and return
